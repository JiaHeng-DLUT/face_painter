...
def save_img(img, output_path, mask):
    result = (img.data.cpu().numpy().transpose((1, 2, 0)) * 255).astype(int)
    # print(result.shape)
    # print(mask.shape)
    if mask is not None:
        result = result * mask
    ...
    if even_idx_y.shape[0] > 0 and even_idx_x.shape[0] > 0:
        for i in range(s):
            ...
            if frame_dir is not None:
                ...
                save_img(frame[0], os.path.join(frame_dir, '%03d.jpg' % idx), original_img)
...
def inference(src_dir, model_path, output_dir, need_animation=False, resize_h=None, resize_w=None, serial=False):
    patch_size = 32
    stroke_num = 8
    ...
    for name in tqdm(os.listdir(src_dir)):
        # try:
        if 1:
            input_path = os.path.join(src_dir, name)
